...
class Commande extends Hydrate {
    ...
    public function addBlankCommande()
    {
        try {
    ...
    public function addTotalDate($id_commande, $prix_comm, $date_commande)
    {
        try {
            $formattedDate = date("Y-m-d", strtotime($date_commande));
            $query = "UPDATE commande SET prix_comm = :prix_comm, date_commande = :date_commande WHERE id_commande = :id_commande";
            $res = $this->_db->prepare($query);
            $res->bindValue(':id_commande', $id_commande);
            $res->bindValue(':prix_comm', $prix_comm);
